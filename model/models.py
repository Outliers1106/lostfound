...
class User(models.Model):
    ...
    #标识：tag=True 为管理员；tag=false 为普通用户
    tag = models.BooleanField(default=False)

class Object(models.Model):
    #物品（物品编号（主）、名称、时间、地点、细节描述、图片、领取状态、标识）
    #物品编号：id django自动生成
    name = models.CharField(max_length=10)
    ...
    #图片：imglink 存放图片的路径
    img  = models.ImageField(default="default/default.jpg",upload_to="img/object")
    #领取状态：state=true已领取;state=false未领取
    state = models.BooleanField(default=False)
    #（lost or found）标识：tag=false lost 失物；tag=true found 拾物
    tag = models.BooleanField(default=False)
...
class TakenRecord(models.Model):
    #认领记录（信息发布者学号、提供失物 / 认领者、物品、认领时间、标识）
    # id（主） django自动生成
    #user1 信息发布者
    ...
    object = models.ForeignKey(Object,
                               on_delete=models.CASCADE,)
    time = models.DateTimeField()
    #（user2）标识 tag=false lost 丢失的人（认领者）；tag=true found 捡到的人
    tag = models.BooleanField(default=False)